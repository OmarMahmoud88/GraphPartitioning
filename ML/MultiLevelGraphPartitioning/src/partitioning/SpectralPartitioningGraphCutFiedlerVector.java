...
public class SpectralPartitioningGraphCutFiedlerVector extends Partitioning {
	...
	public SpectralPartitioningGraphCutFiedlerVector(Graph graph, RandomAccessIntHashSet graphSubset,
	...
	public PartitionGroup getPartitions() {
		...
		for (int i = 0; i < resultMatrix.length; i++) {
			double diagElement1 = ((double) 1) / Math.sqrt(vertixWeightDiagMatrix[i]); // the
																						// matrix
																						// is
																						// not
																						// symmetric
		...
		// Complex64F eigValue =
		// eig.getEigenvalue(eigenValuesindices[this.eigIndex]);
