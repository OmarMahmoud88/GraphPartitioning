...
public class Main {
	...
	final private static HashSet<String> excludedGraphs = new HashSet<String>(Arrays.asList("brack2", "3elt", "wing_nodal", "bcsstk29", "cti", "vibrobox", "whitaker3"));
	final private static int numberOfTrials = 10;
	final private static int numberOfRuns = 100;
	final private static float[] imbalanceRatiosList = new float[] { (float) 0.001999, (float) 0.01, (float) 0.03,
			(float) 0.05, (float) 0.1 };
					...
					while (run_ID <= numberOfRuns) {
						...
						for (int j = 0; j < coarseningClasses.size(); j++) {
							...
							while (intermediate.getNumberOfNodes() > 100
								...
								ArrayList<ArrayList<Integer>> nodesTree = match.coarse(intermediate,
